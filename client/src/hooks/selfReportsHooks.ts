...
export function useGetSelfReports(type: HealthCheckNames) {
  ...
  const selfReport = useSelector(
    (state: AppState) => state.firestore.ordered.selfreports
  )

  // TODO, make more beautiful & Transform the enum values to the same dialog texts.
  if (selfReport && selfReport.length > 0) {
    return selfReport[0].questionaries
      .filter(x => x.questionId === type)
      .map(question => {
...
export function useDispatchSelfReport() {
  ...
  return useCallback(
    async function dispatchSelfReport(reports: AllQuestions[]) {
